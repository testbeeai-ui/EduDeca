...
export function SidebarNav() {
  ...
  return (
    <aside className="fixed inset-y-0 left-0 z-40 hidden h-dvh w-60 flex-col border-r border-white/10 bg-sidebar/80 backdrop-blur-2xl lg:flex shadow-2xl shadow-black/50">
      <div className="flex h-16 shrink-0 items-center border-b border-white/5 px-5">
        <Link href="/home" aria-label="EduDeca home" className="group">
          <EduDecaLogo />
        </Link>
      </div>

      <div className="flex min-h-0 flex-1 flex-col overflow-y-auto overscroll-contain px-3 py-4 dark-scrollbar">
        <nav className="flex shrink-0 flex-col gap-1.5" aria-label="Main navigation">
          {navItems.map((item) => {
        ...
        {!isSignedIn ? (
          <>
            <div
              className="min-h-6 w-full max-h-[400px] flex-1 basis-0"
              aria-hidden
            />
            <ReturningUserLogin className="shrink-0 pb-2" />
          </>
        ) : (
