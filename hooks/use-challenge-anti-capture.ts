...
const EDUDECA_CAPTURE_VEIL_ID = "edudeca-capture-veil";
...
const STUCK_VEIL_RELOAD_SESSION_KEY = "edudeca-anticapture-stuck-reload-once";
/**
...
function destroyImmediateCaptureVeil(): void {
  ...
  const el = document.getElementById(EDUDECA_CAPTURE_VEIL_ID);
  if (el?.parentNode) {
...
function showImmediateCaptureVeil(): void {
  if (typeof document === "undefined") return;
  let el = document.getElementById(EDUDECA_CAPTURE_VEIL_ID) as HTMLDivElement | null;
  if (!el) {
    el = document.createElement("div");
    el.id = EDUDECA_CAPTURE_VEIL_ID;
...
function armImmediateCaptureVeil(): void {
  ...
  if (!el) {
    ...
    el = document.getElementById(EDUDECA_CAPTURE_VEIL_ID) as HTMLDivElement | null;
  }
  if (!el) return;
...
export type UseChallengeAntiCaptureOptions = {
  enabled: boolean;
  clipboardMessage?: string;
};

export function useChallengeAntiCapture({
  enabled,
  clipboardMessage = "Screenshots and screen capture are not available during this challenge.",
}: UseChallengeAntiCaptureOptions) {
