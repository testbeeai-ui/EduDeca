-- Require student state/city to match the approved college when joining roster.
-- Drop prior signature (name-only) so callers must pass location.

DROP FUNCTION IF EXISTS public.edudeca_sync_student_college_roster(
...
BEGIN
  ...
  SELECT EXISTS (
    ...
    WHERE a.status = 'approved'
      ...
      AND lower(btrim(a.state)) = v_state
      AND lower(btrim(a.city)) = v_city
  ) INTO v_matched;
...
REVOKE ALL ON FUNCTION public.edudeca_sync_student_college_roster(
  text, text, text, integer, integer, boolean, text, text, text
) FROM PUBLIC;
...
COMMENT ON TABLE public.edudeca_college_roster IS
  'Students linked to an approved college via matching institution_key and location (state/city).';
